                        ...
                        <div className="flex flex-col md:flex-row justify-between gap-4">
                            ...
                            <div className="shrink-0 text-right">
                                <span className={`px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-widest ${
                                    mail.overall_status === 'approved' ? 'bg-[#51bb2515] text-[#51bb25]' :
                                ...
                                {mail.attachments.map((path, index) => {
                                    ...
                                    return (
                                        <a key={index} href={route('mails.download', { mail: mail.id, index })} className="flex items-center gap-3 p-3 bg-white border border-gray-200 rounded-xl hover:border-indigo-500 hover:shadow-md transition group">
                                            ...
                                            <div>
                                                <div className="text-sm font-bold text-gray-700">{filename.substring(0, 20)}{filename.length > 20 ? '...' : ''}</div>
                                                <div className="text-[10px] text-gray-400 uppercase font-bold tracking-widest">Download</div>
                                            </div>
                                        </a>
                                    );
                                })}
                            ...
                            <div className="space-y-6">
                                {mail.trackers.map((tracker, index) => (
                                    <div key={tracker.id} className="flex gap-4 relative">
                                        ...
                                        <div className={`h-8 w-8 rounded-full flex items-center justify-center shrink-0 z-10 border-2 ${
                                            tracker.status === 'approved' ? 'bg-white border-[#51bb25] text-[#51bb25]' :
